...
export async function sendWelcomeEmail(to: string, adminName: string, choirName: string) {
  try {
    ...
    await resend.emails.send({
      ...
      html: generateWelcomeHtml(adminName, choirName),
    })
  } catch (err) {
...
function generateWelcomeHtml(adminName: string, choirName: string): string {
  return [
    ...
    '<a href="https://choiros.app/login" style="display:inline-block;background:#1a2744;color:#fff;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:600;font-size:15px;margin-bottom:32px">Log in to ChoirOS →</a>',
